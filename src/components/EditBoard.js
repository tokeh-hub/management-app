...
const EditBoard = () => {
  ...
  // Function to close the edit board modal
  ...
  // Function to save the changes made to the board
  const addBoard = async () => {
    try {
      await axios.put(`https://mykanban-onpl.onrender.com/boards/${selectedBoard.id}`, {
        name: boardName,
        columns: columns
      });
      toast('Changes Saved!');
      ...
      <div onClick={e => { e.stopPropagation(); }} className={`w-[300px] xs:w-[500px] sm:w-[600px] h-fit rounded-md ${dark ? 'bg-gray-7 text-white' : 'bg-white text-slate-500'} px-8 py-5 flex flex-col gap-5 relative`}>
          ...
          <div className='w-full'>
            ...
            {columns.map(col => (
              <div key={col.id} className='w-full flex justify-between items-center mb-3'>
                <p
                  className={`border font-bold text-sm rounded-md pl-2 border-slate-500 py-1 px-0.5 w-full mr-3 ${dark ? 'text-white' : 'text-black'}`}
                >
                  {col.name}
                </p>
                <button className={`${dark ? 'text-white' : 'text-slate-500'} font-bold text-xl text-end`} onClick={() => deleteColumn(col.id)}>
                  <FaTimes />
                </button>
              </div>
            ))}
        ...
        {showInput && (
          <div className='w-full flex justify-between items-center gap-3'>
            <button style={{ backgroundColor: color }} className='mt-2 w-fit rounded-md text-black px-2 py-1' onClick={() => setShowColorPicker(!showColorPicker)}>
            ...
            <input
              ...
              className={`mt-2 w-full py-1 px-0.5 text-sm pl-2 ${dark ? 'bg-transparent' : 'bg-white'} outline-none border border-slate-500 rounded-md`}
            />
            <button onClick={addToColumns} className='text-normal bg-purple-1 rounded-md h-fit p-1 mt-1 font-bold text-white text-end'>
              Add
            </button>
          </div>
        )}
