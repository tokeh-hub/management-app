  ...
  return (
    ...
    <div onClick={e => { e.stopPropagation()}} className={`w-[300px] xs:w-[500px] sm:w-[600px] h-fit rounded-md ${dark ? 'bg-gray-7 text-white' :'bg-white text-slate-500'} p-5 flex flex-col gap-5 relative`}>
          ...
          <div>
                ...
                <select value={status} onChange={e=>changeStatus(e)} className={`mt-2 w-full py-1 px-0.5 text-sm pl-2 ${dark ? 'bg-transparent text-white' : 'bg-white text-black'} outline-none border border-slate-500 rounded-md`}>
