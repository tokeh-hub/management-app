          ...
          <div className='column flex flex-col gap-5'>
                ...
                <div className='flex flex-col gap-5'>
                    {column?.tasks.map(task=>(
                        <Task title={task.title} task={task} columns={columns} subtasks={task.subtasks}/>
                    ))}
                </div>
          </div>
       ))}
       <button onClick={()=>showModal()} className={`${dark ? 'bg-gray-7': 'bg-grey-400 hover:text-purple-1'} text-gray-1 font-medium text-md shadow-md mt-10 h-screen rounded-md w-[400px] flex justify-center items-center`}>
                 + New Column
       </button>
    </div>
  )
