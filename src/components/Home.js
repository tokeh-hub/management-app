...
const Home = () => {
  ...
  return (
    <div className={`flex gap-0 relative ${dark ? 'bg-black' : 'bg-gray-3'} w-screen h-screen ${hideSidebar ? 'overflow-y-auto' : 'overflow-hidden'}`}>
      ...
      <div className={`sidebar ${hideSidebar && 'hidden'} sm:w-[300px] md:w-[240px] lg:w-[300px] xl:w-[350px] 2xl:w-[400px]'}`}>
        <SideBar />
      </div>
      <div className={`board ${!hideSidebar && 'sm:max-w-[1300px] h-screen'} sm:w-full h-full overflow-y-auto`}>
        <BoardPage />
      </div>
